...
function MapUpdater({ center, points, mode }: { center: [number, number], points: HistoryPoint[], mode: string }) {
  const map = useMap();
  useEffect(() => {
    if (mode === 'history' && points && points.length > 1) {
...
export default function TrackingPage() {
  ...
  return (
    <AppLayout allowedRoles={['admin']}>
          ...
          {mode === 'history' && (
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <input
                ...
                style={{ padding: '6px 12px', fontSize: '13px', height: 'auto' }}
              />
              {loadingHistory && <div className="spinner" style={{ width: 16, height: 16, borderWidth: 2 }} />}
            </div>
          )}
      ...
      {mode === 'history' && !loadingHistory && selectedUserId && historyPoints.length === 0 && (
        <div style={{ padding: '8px 16px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', borderRadius: '8px', marginBottom: '10px', fontSize: '13px', fontWeight: 600 }}>
          {fr ? "Aucun historique GPS trouvé pour cet utilisateur à cette date." : "لم يتم العثور على سجل GPS لهذا المستخدم في هذا التاريخ."}
        </div>
      )}
      ...
      <div style={{
        ...
        <MapContainer center={center} zoom={12} style={{ height: '100%', width: '100%' }}>
          ...
          />
          <MapUpdater center={center} points={historyPoints} mode={mode} />
            ...
            <>
              ...
              >
                <Tooltip permanent direction="bottom" offset={[0, 5]}>
                  <span style={{ fontWeight: 'bold' }}>{fr ? 'Départ' : 'الإنطلاق'}</span>
                </Tooltip>
                <Popup>{fr ? 'Départ:' : 'الإنطلاق:'} {new Date(historyPoints[0].timestamp).toLocaleTimeString()}</Popup>
              </Marker>
              ...
              {historyPoints.length > 1 && (
                ...
                >
                  <Tooltip permanent direction="bottom" offset={[0, 5]}>
                    <span style={{ fontWeight: 'bold' }}>{fr ? 'Arrivée' : 'الوصول'}</span>
                  </Tooltip>
                  <Popup>{fr ? 'Dernière position:' : 'آخر موقع:'} {new Date(historyPoints[historyPoints.length - 1].timestamp).toLocaleTimeString()}</Popup>
                </Marker>
              )}
            </>
          )}
